...
public class AndesiteCrateScreen extends AbstractDoubleStorageScreen<AndesiteCrateMenu> {
    ...
    protected void init() {
        ...
        setWindowOffset(menu.dualBlock ? -4 : 0, - 7);
        super.init();
        clearWidgets();

        itemLabelOffset = menu.dualBlock ? 137 : 65;
        ...
        allowedItems = new ScrollInput(x + itemLabelOffset, y + 104, 41, 16).titled(storageSpace.plainCopy())
                .withRange(0, (menu.dualBlock ? 2049 : 1025))
    ...
    public void renderBg(@NotNull GuiGraphics ms, float partialTicks, int mouseX, int mouseY) {
        int invX = getLeftOfCentered(PLAYER_INVENTORY.getWidth()) + windowXOffset + (menu.dualBlock ? 10 : 0);
